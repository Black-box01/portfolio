...
export default function TESTIMONIALSUpdateForm(props) {
  const {
    id,
    tESTIMONIALS,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...tESTIMONIALSRecord };
    setImg(cleanValues.img);
    setReview(cleanValues.review);
    setErrors({});
  };
  const [tESTIMONIALSRecord, setTESTIMONIALSRecord] =
    React.useState(tESTIMONIALS);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id
        ? await DataStore.query(TESTIMONIALS, id)
        : tESTIMONIALS;
      setTESTIMONIALSRecord(record);
    };
    queryData();
  }, [id, tESTIMONIALS]);
  React.useEffect(resetStateValues, [tESTIMONIALSRecord]);
  const validations = {
    img: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            TESTIMONIALS.copyOf(tESTIMONIALSRecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "TESTIMONIALSUpdateForm")}
    >
      <TextField
        label="Img"
        isRequired={true}
