...
export default function INTROesCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    ...
    setCharacterImage(initialValues.characterImage);
    setBlueBkImage(initialValues.blueBkImage);
    setOrangeBkImage(initialValues.orangeBkImage);
    setErrors({});
  };
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(new INTROes(modelFields));
          if (onSuccess) {
      ...
      {...rest}
      {...getOverrideProps(overrides, "INTROesCreateForm")}
    >
      <TextField
        label="First text"
        isRequired={true}
