...
export default function PORTFOLIOPHONEUpdateForm(props) {
  const {
    id,
    pORTFOLIOPHONE,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...pORTFOLIOPHONERecord };
    setImgPhone(cleanValues.imgPhone);
  ...
  const [pORTFOLIOPHONERecord, setPORTFOLIOPHONERecord] =
    React.useState(pORTFOLIOPHONE);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id
        ? await DataStore.query(PORTFOLIOPHONE, id)
        : pORTFOLIOPHONE;
      setPORTFOLIOPHONERecord(record);
    };
    queryData();
  }, [id, pORTFOLIOPHONE]);
  React.useEffect(resetStateValues, [pORTFOLIOPHONERecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            PORTFOLIOPHONE.copyOf(pORTFOLIOPHONERecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "PORTFOLIOPHONEUpdateForm")}
    >
      <TextField
        label="Img phone"
        isRequired={true}
        isReadOnly={false}
        defaultValue={imgPhone}
        onChange={(e) => {
          let { value } = e.target;
          if (onChange) {
            const modelFields = {
