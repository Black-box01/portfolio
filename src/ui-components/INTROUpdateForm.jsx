...
export default function INTROUpdateForm(props) {
  const {
    id,
    iNTRO,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...iNTRORecord };
    ...
    setHireMeLink(cleanValues.hireMeLink);
  ...
  const [iNTRORecord, setINTRORecord] = React.useState(iNTRO);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(INTRO, id) : iNTRO;
      setINTRORecord(record);
    };
    queryData();
  }, [id, iNTRO]);
  React.useEffect(resetStateValues, [iNTRORecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            INTRO.copyOf(iNTRORecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "INTROUpdateForm")}
    >
      <TextField
        label="First text"
        isRequired={true}
